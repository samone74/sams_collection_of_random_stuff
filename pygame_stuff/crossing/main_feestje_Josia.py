...
# defining collors to be used later
    ...
    [CARHEIGHT,CARWIDTH, road0bj, car0bj,Player0bj,NIGHT,NLANES, PLAYER]=start(WINHEIGHT,WINWIDTH,PLAYERHEIGHT,Player0bj)
    ...
    keypressed = 0
    wrong = False
    message = "Type de ballon code in "
    while True:
        for event in pygame.event.get():
            ...
            if event.type == QUIT:
                ...
                if (event.key == K_ESCAPE):
                    terminate()
                elif(event.key == K_5)| (event.key == K_KP5):
                    if keypressed == 0:
                        K1 = True
                    else:
                        wrong = True
                    message += "5"
                    keypressed += 1
                elif((event.key == K_6) | (event.key == K_KP6) ):
                    if K1:
                        K2 = True
                    else:
                        wrong = True
                    message += "6"
                    keypressed += 1
                elif((event.key == K_4) | (event.key == K_KP4)):
                    if K2:
                        K3 = True
                    else:
                        wrong = True
                    message += "4"
                    keypressed += 1
                elif((event.key == K_2) | (event.key == K_KP2)):
                    if K3:
                        K4 = True
                    message += "2"
                    keypressed += 1
                elif ((event.key == K_3) | (event.key == K_KP3)):
                    if K4:
                        K5 = True
                    else:
                        wrong = True
                    message += "3"
                    keypressed += 1
                elif ((event.key == K_1) | (event.key == K_KP1)):
                    if K5:
                        K6 = True
                    else:
                        wrong = True
                    message += "1"
                    keypressed += 1
                elif ((event.key == K_7) | (event.key == K_KP7)):
                    message += "7"
                    keypressed += 1
                    wrong = True
                elif ((event.key == K_8) | (event.key == K_KP8)):
                    message += "8"
                    keypressed += 1
                    wrong = True
                elif ((event.key == K_9) | (event.key == K_KP9)):
                    message += "9"
                    keypressed += 1
                    wrong = True
        if K6:
    ...
    while True:
        ...
        [CARHEIGHT,CARWIDTH, road0bj, car0bj,NLANES,NIGHT,restarting,PLAYER]=checkcolission(Player0bj,car0bj,road0bj,PLAYERHEIGHT,PLAYERWIDTH,CARHEIGHT,CARWIDTH,NLANES,WINHEIGHT,WINWIDTH,NIGHT,PLAYER)
        if restarting:
            restarting = False
            Moveleft  = False
            Movedown  = False
    ...
    for car in car0bj:
        if Player0bj['x'] +PLAYERWIDTH > car['x'] and Player0bj['x'] < car['x']+CARWIDTH:
            if Player0bj['y']+PLAYERHEIGHT> car['y'] and Player0bj['y'] < car['y']+CARHEIGHT:
                ...
                Player0bj['lives'] = Player0bj['lives']
                if android:
                    android.vibrate(1)    
    ...
    if Player0bj['y'] == 0:
        ...
        if NLANES > 5:
            ...
            if PLAYER > 4:
                ...
                [CARHEIGHT,CARWIDTH, road0bj, car0bj,Player0bj,NIGHT,NLANES]=winlose(CARHEIGHT,CARWIDTH, road0bj, car0bj,Player0bj,NIGHT,NLANES,WINHEIGHT,WINWIDTH,PLAYERHEIGHT,message)
                exit()
            message = 'De volgende speler is'
            [CARHEIGHT, CARWIDTH, road0bj, car0bj, Player0bj, NIGHT, NLANES] = winlose(CARHEIGHT, CARWIDTH, road0bj,
                                                                                       car0bj, Player0bj, NIGHT, NLANES,
                                                                                       WINHEIGHT, WINWIDTH,
                                                                                       PLAYERHEIGHT, message)

        [CARHEIGHT,CARWIDTH, road0bj, car0bj]=makelanesandcars(NLANES,WINHEIGHT,WINWIDTH)
    return CARHEIGHT,CARWIDTH, road0bj, car0bj, NLANES, NIGHT, restarting, PLAYER
...
def start(WINHEIGHT,WINWIDTH,PLAYERHEIGHT,Player0bj):
    ...
    return  CARHEIGHT,CARWIDTH, road0bj, car0bj,Player0bj,NIGHT,NLANES, PLAYER
